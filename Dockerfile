...
RUN git clone https://github.com/PaddlePaddle/Paddle.git ~/Paddle && cd ~/Paddle && git checkout v0.14.0 && \
    ...
    cmake -DCMAKE_BUILD_TYPE=Release -DWITH_GPU=ON -DWITH_FLUID_ONLY=ON .. && \
    make -j 8
...
# # debug
# FROM paddle:lac
# RUN cd ~/lac && git pull && git checkout for_paddle_v1.1 && rm -rf build && mkdir build && cd build && cmake -DPADDLE_ROOT=/root/Paddle/cbuild/fluid_install_dir .. && \
#     make && make install && \
#     pip install tqdm && \
#     chmod u+x ~/lac/python/infer_fifo.py
